...
- (void)storePurchase:(NSString *)sku withReceipt:(NSString *)receipt
{
    NSUserDefaults *standardUserDefaults = [NSUserDefaults standardUserDefaults];
    if (standardUserDefaults)
    {
        [standardUserDefaults setObject:receipt forKey:sku];
...
- (void)queryInventory
{
    ...
    if (!standardUserDefaults)
        ...
        {
            NSString* receipt = [standardUserDefaults objectForKey:sku];
            if (receipt != nil)
            {
                ...
                NSArray* entry = [NSArray arrayWithObjects:sku, jsonString, nil];
    ...
    for (SKPaymentTransaction *transaction in transactions)
    {
        switch (transaction.transactionState)
        {
            ...
            case SKPaymentTransactionStateRestored:
                [self storePurchase:transaction.payment.productIdentifier withReceipt:[transaction.transactionReceipt base64Encoding]];
